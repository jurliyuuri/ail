...
function wordParser(word) {
    const [id, entryForm, rawWordClasses, linzi, rawMeanings, rawRelatives, rawRelIDs, rawRelTags, doublets, relativeLip, example, comment] = word.split('\t');
    return [
        Number(id),
        entryForm,
...
const words = lines.slice(1).map(word => {
    const [id, entryForm, wordClasses, linzi, meanings, relatives, relIDs, relTags, doublets, relativeLip, example, comment] = wordParser(word);
    const entry = {
        "id": id,
        "form": entryForm
    };
